...
describe('element methods', () => {
  ...
  it('getWebElement', () => {
    ...
    // The other method is the locator value that Protractor uses to look up values.
    console.log('not the same as WebElement.getLocator = ' +
  ...
  it('isPresent and isElementPresent', () => {
    ...
    // fill out the name field. the Angular structural directive *ngIf will change the dom
    // and the contact email field will appear.
  ...
  it('getTagName', () => {
    ...
    // Sometimes we have a web element and want to query for a web element that exists within
    // its children. In this case, we are searching for the material toolbar via the color="primary"
    // attribute. The value of the getTagName should be the material tag name for the toolbar.
    let body = element(by.tagName('body'));
  ...
  it('getAttribute', () => {
    ...
    // Check the keys we entered are as expected. Note that you cannot use getText() on an input
  ...
  it('getText', () => {
    ...
    // This is the content of tbody. This is difficult to test a single row but we can check
    // to see if the contents match with regex.
  ...
  it('isDisplayed', () => {
    ...
    browser.executeScript("arguments[0].setAttribute('style', 'display:none;')",
